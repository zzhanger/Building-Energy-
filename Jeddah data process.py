# -*- coding: utf-8 -*-
"""
Created on Thu Jun 23 16:20:29 2022
...
wb = Workbook()
ws1 = wb.create_sheet("JD",index=0)
...
df_1=pd.read_excel('Floor space calculation.xlsx',sheet_name='Jeddah',usecols='A,B,C,D,E,F,G')
Building_JD = df_1[df_1['FLOOR AREA (m2)']>0]
# Building_JD_D = Building_JD[(Building_JD['BUILDING TYPE'] == 'apartments')|(Building_JD['BUILDING TYPE'] == 'house')|(Building_JD['BUILDING TYPE'] == 'residential')|(Building_JD['BUILDING TYPE'] == 'yes')]
#GFA_D = Building_JD_D['FLOOR AREA (m2)'].sum()
df_2 =pd.read_excel('Distance from building to coastline.xlsx',sheet_name='JD') # distance from buildings to coastline (m)
df_3=pd.merge(Building_JD,df_2,how='inner',on=["ID"])
Population_density = pd.read_excel('Population density.xlsx',sheet_name='JD') # population density (capita/km2)
...
file_path='C:\EnergyPlusV9-6-0\Residential building\Residential buildings results\Jeddah\Monthly load\\' 
...
Discount_rate = 0.025  #senstivity parameter +-10%
lifespan = 60 #senstivity parameter +-10%
ele_price = 0.048 #senstivity parameter +-10%
...
cost_each_building_DCS = (f* (Cooling_load_required**0.87))*0.67 # 0.87 is the scaling factor 
...
annual_AC_output = 7347 #kWh per 7kW panel (50 m2 each)
annual_ele_gain_PV = (df_5['FOOTPRINT AREA (m2)']*suitable_ratio)/110*annual_AC_output


cost_each_building_PV = 312*(df_5['FOOTPRINT AREA (m2)']*suitable_ratio)**0.87*0.72
lump_sum_PV = annual_ele_gain_PV*ele_price-cost_each_building_PV*0.001*0.72
...
pum_eff = 0.6
GHG_factor = 0.51 # kg CO2e/kWh adjustable
...
# data_2 = df_10['ee_DCS (USD/kgCO2e)'].fillna(0)
# # data_1.corr(data_2)
# stats.pearsonr(data_1,data_2)