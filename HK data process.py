# -*- coding: utf-8 -*-
"""
Created on Thu Jun 23 11:20:33 2022
...
wb = Workbook()
ws1 = wb.create_sheet("HK",index=0)
...
df_1=pd.read_excel('Floor space calculation.xlsx',sheet_name='HK',usecols='A,B,C,D,E,F,G')
Building_HK = df_1[df_1['FLOOR AREA (m2)']>0]
# Building_HK_D = Building_HK[(Building_HK['BUILDING TYPE'] == 'apartments')|(Building_HK['BUILDING TYPE'] == 'house')|(Building_HK['BUILDING TYPE'] == 'residential')|(Building_HK['BUILDING TYPE'] == 'yes')]
#GFA_D = Building_HK_D['FLOOR AREA (m2)'].sum()
df_2 =pd.read_excel('Distance from building to coastline.xlsx',sheet_name='HK') # distance from buildings to coastline (m)
df_3=pd.merge(Building_HK,df_2,how='inner',on=["ID"])
Population_density = pd.read_excel('Population density.xlsx',sheet_name='HK') # population density (capita/km2)
...
file_path='C:\EnergyPlusV9-6-0\Residential building\Residential buildings results\HK\Monthly load\\' 
...
Discount_rate = 0.0175  #senstivity parameter +-10%
lifespan = 60 #senstivity parameter +-10%
ele_price = 0.15 #senstivity parameter +-10%
...
cost_each_building_DCS = (f* (Cooling_load_required**0.87)) # 0.83 is the scaling factor 
...
annual_AC_output = 5111 #kWh per 7kW panel (50 m2 each)
annual_ele_gain_PV = (df_5['FOOTPRINT AREA (m2)']*suitable_ratio)/110*annual_AC_output


cost_each_building_PV = 312*(df_5['FOOTPRINT AREA (m2)']*suitable_ratio)**0.9*1.07
...
pum_eff = 0.6
GHG_factor = 0.71 # kg CO2e/kWh adjustable
...
# df_10.to_excel('HK data.xlsx')

#%% SWDCS and SPVS senstivity analysis
